...
**Conferido por comando nesta data:** 46 arquivos de teste, 492 casos, 43
componentes (`vitest run --pool=threads`, em série; `ls src/components`).
*A SPEC-052 registrou 43/473/42; a SPEC-053 soma três arquivos de teste e um
componente (`nova-reserva`).*
...
| `/home` | `home-view` | próximas aulas e atalhos. **SPEC-053/D4 — a Home não fala mais em Quadras:** três atalhos, nesta ordem, Reservar (`/reservas/nova`), Aulas e Reservas; o destaque e o cartão "Reservas PlayCK" levam a `/reservas/nova`, e **nenhum link aponta para `/quadras`** (AC-009). Quadra virou um *tipo* de reserva (ADR-021), e a Home que oferecesse "Quadras" continuaria vendendo um tipo como se fosse a área |
...
| `/quadras/[id]` | `court-booking` | reservar UMA quadra. **Não** foi afetada pelo redirect do índice: é o passo seguinte do fluxo, não uma aba — e segue assim na SPEC-053 (AC-011, medido: `200`, sem `Location`). A frase *"A quadra não abre neste dia."* fica: fala da quadra física, não da área (categoria C da D1) |
